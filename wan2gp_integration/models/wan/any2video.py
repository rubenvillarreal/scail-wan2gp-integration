...
# Lazy imports for other models (not used by SCAIL)
...
class WanAny2V:
    ...
    def generate(self,
        ...
        **bbargs
        ...
        # Wan-Move
        if wanmove:
            # Lazy import for wanmove (not used by SCAIL)
            from .wanmove.trajectory import replace_feature, create_pos_feature_map
        ...
        if apg_switch != 0:
            # Lazy import for multitalk APG (not used by SCAIL)
        ...
        if color_correction_strength > 0 and (window_start_frame_no + prefix_frames_count) >1:
            # Lazy import for multitalk color matching (not used by SCAIL)
            from .multitalk.multitalk_utils import match_and_blend_colors, match_and_blend_colors_with_mask
